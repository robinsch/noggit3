...
namespace noggit
{
    namespace ui
    {
            ...
            , pasteMode(PASTE_ON_TERRAIN)
        {
            ...
            QPushButton* from_last_map = new QPushButton("From Last Map", this);
            ...
            importBox->layout()->addWidget(from_last_map);
        ...
        void object_editor::copy_global()
        {
            std::vector<selection_type> selected_model;
            ...
            for (auto& selection : main_window::instance()->selected_global)
            {
                ...
                {
                    ModelInstance* mi = new ModelInstance(filename);
                    mi->scale = selection.scale;
                    mi->dir = selection.dir;
                    mi->pos = selection.pos;
                ...
                {
                    WMOInstance* wi = new WMOInstance(filename);
                    wi->dir = selection.dir;
                    wi->pos = selection.pos;
        ...
        void object_editor::import_last_map()
        {
            copy_global();
        }
